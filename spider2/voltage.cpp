...
namespace SPIDER {
   ...
   * 
   * Devuelve el valor en voltios usando la referencia de 5.0 V
   *               1023  ----> 5 V
   *                  r  ----> result
   * 
   * Se multiplica por 2 para dar el valor del voltage de entrada
   * al divisor de tension.
   * 
   ****************************************************************/
  float Voltage::getVoltage() const {
    int r = analogRead(VOL_MEASURE_PIN);
    
    return 2.0 * (5.0 * r) / 1023.0;
  }
}
