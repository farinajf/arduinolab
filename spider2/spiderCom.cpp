...
namespace SPIDER {
  ...
  void updateService() {
    ...
    {
      communication -> getController().getRobot().update();
  ...
  void SpiderCom::init(bool x = true) {
    ...
    //6.-
    if (_controller.getRobot().getPower().isPowerGroupAutoSwitch()) delay(1000);
  }
  ...
  void SpiderCom::_handleOrder(byte data[], OrderSource orderSource) {
    ...
    {
      _controller.getRobot().calibrateServos();
  ...
  void SpiderCom::_setRobotBootState(RobotState state) {
    if      (state == RobotState::INSTALL)   _controller.getRobot().installState();
    else if (state == RobotState::CALIBRATE) _controller.getRobot().calibrateState();
    else if (state == RobotState::BOOT)      _controller.getRobot().bootState();
  }
  ...
  void SpiderCom::_updateAutoSleep() {
    ...
    if (_controller.getRobot().getState() == RobotState::ACTION) _controller.setSleepMode();
  ...
  void SpiderCom::_updateBlockedOrder() {
    ...
    else if (blockedOrder == Orders::REQ_ACTIVE_MODE)         // 92
    {
      ...
      _controller.setActiveMode();
    }
    else if (blockedOrder == Orders::REQ_SLEEP_MODE)          // 94
    {
      this -> _saveRobotBootState(RobotState::BOOT);
      
      _controller.setSleepMode();
    }
    ...
    else if (blockedOrder == Orders::REQ_CHANGE_BODY_HEIGHT)  //112
    {
      ...
      _controller.changeBodyHeight(-h);
    }
  ...
  void SpiderCom::_updateStateLED() {
    ...
    if (_ledCounter / _LED_BLINK_CYCLE >= abs(_ledState) + (unsigned int) 3)
    {
      ...
      if      (_controller.getRobot().getState() == RobotState::ACTION)    _ledState = 1;
      else if (_controller.getRobot().getState() == RobotState::BOOT)      _ledState = 1;
      else if (_controller.getRobot().getState() == RobotState::CALIBRATE) _ledState = 2;
      else if (_controller.getRobot().getState() == RobotState::INSTALL)   _ledState = 3;

      if ((_controller.getRobot().getPower().isPowerGroupAutoSwitch() == true) && (_controller.getRobot().getPower().isPowerGroupState() == false)) _ledState = -1;
    }
  }
}
